...
step_display = 50
step_save = 10000
path_save = '../../save/vgg-exp1'
num = 40000 #the model chosen to run on test data
start_from = '../../save/vgg-exp1-'+str(num)
train = True;
validation = True;
test = False;
...
with tf.Session() as sess:
    ...
    # Evaluate model
    accuracy1 = tf.reduce_mean(tf.cast(tf.nn.in_top_k(vgg, y, 1), tf.float32))
    accuracy5 = tf.reduce_mean(tf.cast(tf.nn.in_top_k(vgg, y, 5), tf.float32))
    ...
    if test:
        ...
        for i in range(num_batch):
            ...
            l = sess.run([vgg], feed_dict={x: images_batch, y: labels_batch, keep_dropout: 1., train_phase: False})
            l = np.array(l)
