...
def batch_norm(x, n_out, train_phase, scope='bn'):
    """
...
def res_new(_X, _weights, _biases, _bnorm):
    ...
    _X = tf.reshape(_X, shape=[-1, 126, 126, 3])
...
with tf.Session() as sess:
    ...
    if train:
        ...
        evl_size=32
        while 1==1:
            saver.restore(sess,"resnew-30000")
            ...
            #acc5 = sess.run(top5, feed_dict={x: batch_xs, y: batch_ys})
            ans = sess.run(pred, feed_dict={x: batch_xs, y: batch_ys, train_phase: False})
            print(ans)
